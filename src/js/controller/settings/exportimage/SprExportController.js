(function () {
  ...
  ns.SprExportController = function (piskelController, exportController) {
    this.piskelController = piskelController;
    this.exportController = exportController;
    this.onScaleChanged_ = this.onScaleChanged_.bind(this);
  };

  pskl.utils.inherit(ns.SprExportController, pskl.controller.settings.AbstractSettingController);

  ns.SprExportController.prototype.init = function () {
    this.layoutContainer = document.querySelector('.spr-export-layout-section');
    this.dimensionInfo = document.querySelector('.spr-export-dimension-info');

    this.rowsInput = document.querySelector('#spr-export-rows');
    this.columnsInput = document.querySelector('#spr-export-columns');

    var downloadButton = document.querySelector('.spr-download-button');
  ...
  ns.SprExportController.prototype.destroy = function () {
   ...
   */
  ns.SprExportController.prototype.initLayoutSection_ = function () {
  ...
  ns.SprExportController.prototype.updateDimensionLabel_ = function () {
  ...
  ns.SprExportController.prototype.getColumns_ = function () {
    return parseInt(this.columnsInput.value || 1, 10);
  };

  ns.SprExportController.prototype.getRows_ = function () {
    return parseInt(this.rowsInput.value || 1, 10);
  };

  ns.SprExportController.prototype.getBestFit_ = function () {
  ...
  ns.SprExportController.prototype.onScaleChanged_ = function () {
    this.updateDimensionLabel_();
  };
   ...
   */
  ns.SprExportController.prototype.onColumnsInput_ = function () {
  ...
  ns.SprExportController.prototype.createPngSpritesheet_ = function () {
  ...
  ns.SprExportController.prototype.onDownloadClick_ = function (evt) {
    // Create PNG export.
    ...
    var byteArray = new Uint8Array(canvas.width * canvas.height);
    ...
    for (var y = 0; y < canvas.height; y++)
    {
        for (var x = 0; x < canvas.width; x++)
        {
            ...
            {
              rgb = (red / 32 << 5) + (green / 32 << 2) + (blue / 64);
            }
            byteArray[n++] = rgb;
        } 
    }
  ...
  ns.SprExportController.prototype.downloadCanvas_ = function (canvas) {
    // Generate file name
    var name = this.piskelController.getPiskel().getDescriptor().name;
  ...
  ns.SprExportController.prototype.onPixiDownloadClick_ = function () {
  ...
  ns.SprExportController.prototype.onDataUriClick_ = function (evt) {
